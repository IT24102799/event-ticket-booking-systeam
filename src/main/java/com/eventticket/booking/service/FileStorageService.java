...
public class FileStorageService {
    ...
    public List<Admin> readAdmins() {
        System.out.println("FileStorageService.readAdmins() called");
        try {
            File file = new File(DATA_DIR + File.separator + ADMINS_FILE);
            System.out.println("Checking for admins file at: " + file.getAbsolutePath());
            if (!file.exists()) {
                System.out.println("Admins file does not exist, returning empty list");
                return new ArrayList<>();
            }

            System.out.println("Reading admins from file");
            Admin[] admins = objectMapper.readValue(file, Admin[].class);
            List<Admin> adminList = new ArrayList<>();
            for (Admin admin : admins) {
                adminList.add(admin);
                System.out.println("Read admin: " + admin.getAdminId() + ", " + admin.getUserName());
            }
            System.out.println("Returning " + adminList.size() + " admins");
            return adminList;
        } catch (IOException e) {
    ...
    public void writeAdmins(List<Admin> admins) {
        System.out.println("FileStorageService.writeAdmins() called with " + admins.size() + " admins");
        ...
        try {
            ...
            File file = new File(DATA_DIR + File.separator + ADMINS_FILE);
            System.out.println("Writing admins to file: " + file.getAbsolutePath());
            objectMapper.writeValue(file, admins);
            System.out.println("Successfully wrote " + admins.size() + " admins to file");
        } catch (IOException e) {
